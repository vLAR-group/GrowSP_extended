...
def get_sp_feature(args, loader, current_sp, model=None):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            if current_sp is not None:
                ...
                sp_idx = KMeans(n_clusters=n_segments, n_init=10, random_state=0).fit_predict(region_feats.cpu().numpy())
            ...
            context.append((scene_name, gt, raw_region, inverse_map[0]))
...
def get_sp_feature_unScenes(args, loader, current_sp, model=None):
    print('computing point feats ....')
    point_feats_list = []
    point_labels_list = []
    all_sp_index = []
...
# def get_sp_feature_urban(args, loader,current_sp, model=None):
#     print('computing point feats ....')
#     point_feats_list = []
#     point_labels_list = []
#     all_sp_index = []
...
def get_pseudo(args, context, cluster_pred, all_sp_index=None):
    ...
    pseudo_label_folder = os.path.join(args.save_path, args.pseudo_label_path)
    ...
    for i in range(len(context)):
        scene_name, labels, region, inv = context[i]
        ...
        pseudo_label_file = os.path.join(pseudo_label_folder, scene_name + '.npy')
        np.save(pseudo_label_file, pseudo[inv])
