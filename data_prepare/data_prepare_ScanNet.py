...
parser.add_argument('--data_path', type=str, default='./data/scannet/raw', help='raw data path')
parser.add_argument('--save_path', type=str, default='./data/ScanNet/processed')
parser.add_argument('--sp_path', type=str, default='./data/ScanNet/Felzenszwalb')
parser.add_argument('--Felzenszwalb', type=bool, default=False)
args = parser.parse_args()

if args.Felzenszwalb:
    os.makedirs(args.processed_sp_path, exist_ok=True)
...
BUGS = {'scene0270_00': 50, 'scene0270_02': 50, 'scene0384_00': 149}
...
def handle_process(path):
    f = Path(path.split(',')[0])
    if args.Felzenszwalb:
        '''Felzenszwalb superpoints'''
        segment_indexes_filepath = next(f.parent.glob("*[0-9].segs.json"))
        with open(segment_indexes_filepath) as file:
    ...
    if args.Felzenszwalb:
        np.save(os.path.join(args.processed_sp_path, f.name[:-len(POINTCLOUD_FILE)]+'_superpoint.npy'), segment_ids)
