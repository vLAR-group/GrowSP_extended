...
parser.add_argument('--input_path', type=str, default='../data/S3DIS/unalign', help='raw data path')
parser.add_argument('--save_path', type=str, default='../data/S3DIS/initial_sp')
...
def construct_superpoints(path):
    ...
    grids, unique_map, inverse_map = voxelize(coords)
    grids = grids.astype(np.float32)
    coords, feats, labels = coords[unique_map], feats[unique_map], labels[unique_map]

    '''VCCS'''
    out = supervoxel_clustering(grids, feats)
    ...
    for voxel in range(len(out)):
        if out[voxel][1].voxels_.xyz.shape[0] >= 0:
            for xyz_voxel in out[voxel][1].voxels_.xyz:
                index_colum = np.where((xyz_voxel == grids).all(1))
                voxel_idx[index_colum] = voxel_num
            voxel_num += 1

    '''Region Growing'''
    clusters = region_growing_simple(grids)[0]
    ...
    np.save(args.save_path + '/' + f.name[:-4] + '_superpoint.npy', out_sp_labels)

    if vis:
        vis_path = args.save_path +'/vis/'
