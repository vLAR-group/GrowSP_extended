...
def eval(epoch, args):
    model = Res16FPN18(in_channels=6).cuda()
...
def test_once(matching, model, test_loader, classifier, use_sp=False):
    for data in test_loader:
        with torch.no_grad():
            ...
            write_ply('online_testing/vis/' + scene_name + '.ply', [coords[inverse_map[0]], colors], ['x', 'y', 'z', 'red', 'green', 'blue'])
...
def online_test(epoch, args):
    ...
    primitive_centers = cls.weight.data  ###[300, 128]
    print('Merging Primitives')
    cluster_pred = KMeans(n_clusters=args.semantic_class, n_init=10, random_state=0, n_jobs=-1).fit_predict(primitive_centers.cpu().numpy())  # .astype(np.float64))
