...
def vis_preds(epoch, args):
    ...
    # cls = torch.nn.Linear(args.feats_dim, 12, bias=False).cuda()
    cls.load_state_dict(torch.load('/home/zihui/SSD/GrowSP_newExt/ckpt_seg/S3DIS_Area6/primitive_grow/distillv2_80-20sp_kmeans_elsaug_t3_fixmodel_0.9_recls/cls_300_checkpoint.pth'))
    cls.eval()
    ############################ PICIE ##########################
    # model = Res16FPN18(in_channels=args.input_dim, out_channels=20, conv1_kernel_size=args.conv1_kernel_size, config=args).cuda()
    # model.load_state_dict(torch.load('/home/zihui/SSD2/GrowSP_extension/PICIE/PICIE-S_PFH/Area5/model_120_checkpoint.pth'))
    # model.eval()
    #
    # cls = torch.nn.Linear(args.feats_dim, 300, bias=False).cuda()
    # cls.load_state_dict(torch.load('/home/zihui/SSD2/GrowSP_extension/PICIE/PICIE-S_PFH/Area5/cls1_120_checkpoint.pth'))
    # cls.eval()
    ############################## IIC ###########################
    # model = Res16FPN18(in_channels=args.input_dim, out_channels=20, conv1_kernel_size=args.conv1_kernel_size, config=args).cuda()
    # model.classifier = torch.nn.Linear(128+13, 300, bias=False).cuda()
    # model.classifier2 = torch.nn.Linear(128+13, 3*12, bias=False).cuda()
    # model.load_state_dict(torch.load('/home/zihui/SSD2/GrowSP_extension/IIC/IIC-S_PFH/Area5/model_50_checkpoint.pth'))
    # model.eval()
    #
