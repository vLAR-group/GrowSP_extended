...
def parse_args():
    ...
    parser.add_argument('--sp_path', type=str, default='../data/ScanNet/initial_superpoints_0.25/',
                        help='initial sp path')
    parser.add_argument('--vis_path', type=str, default='vis_predications/ScanNet/',
                        help='model savepath')
    ###
...
def vis_preds(args, model_path, cls_path):
    model = Res16FPN18(in_channels=args.input_dim).cuda()
    ...
    for data in trainval_loader:
        with torch.no_grad():
            coords, features, inverse_map, voxel_label, index, region, pc, color, label = data
            ...
            preds_full = preds[inverse_map[0]]
            voxel_preds.append(preds), voxel_labels.append(voxel_label)
            all_full_preds.append(preds_full), all_full_labels.append(label), all_full_coords.append(pc), all_full_colors.append(color)
    ...
    print('Test acc: {:.5f}  Test IoU'.format(o_Acc), s)
...
if __name__ == '__main__':
    args = parse_args()
    model_path = 'ckpt_seg/ScanNet/model_300_checkpoint.pth'
    cls_path = 'ckpt_seg/ScanNet/cls_300_checkpoint.pth'
