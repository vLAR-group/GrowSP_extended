...
def get_sp_feature(args, loader, model0, model1, current_growsp):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            in_field0, in_field1 = ME.TensorField(features0[:, 0:3], coords0, device=0), ME.TensorField(features1[:, 3:], coords1, device=0)
            feats0, feats1 = model0(in_field0), model1(in_field1)
            ...
            if current_growsp is not None:
                ...
                sp_idx0 = torch.from_numpy(KMeans(n_clusters=n_segments, n_init=5, random_state=0, n_jobs=5).fit_predict(region_feats0.cpu().numpy())).long()
                sp_idx1 = torch.from_numpy(KMeans(n_clusters=n_segments, n_init=5, random_state=0, n_jobs=5).fit_predict(region_feats1.cpu().numpy())).long()
            else:
                feats0, feats1 = region_feats0, region_feats1
                sp_idx0 = sp_idx1 = torch.tensor(range(region_feats0.size(0)))

            neural_region0, neural_region1 = sp_idx0[region], sp_idx1[region]
            pfh0, pfh1 = [], []

            neural_region_num0 = len(torch.unique(neural_region0))
            neural_region_corr0 = torch.zeros(neural_region0.size(0), neural_region_num0)
            neural_region_corr0.scatter_(1, neural_region0.view(-1, 1), 1)
            neural_region_corr0 = neural_region_corr0.cuda()
            per_neural_region_num0 = neural_region_corr0.sum(0, keepdims=True).t()

            neural_region_num1 = len(torch.unique(neural_region1))
            neural_region_corr1 = torch.zeros(neural_region1.size(0), neural_region_num1)
            neural_region_corr1.scatter_(1, neural_region1.view(-1, 1), 1)
            neural_region_corr1 = neural_region_corr1.cuda()
            per_neural_region_num1 = neural_region_corr1.sum(0, keepdims=True).t()
            #
            '''Compute avg rgb/pfh for each Superpoints to help Primitives Learning'''
            final_rgb0, final_rgb1 = F.linear(neural_region_corr0.t(), pc_rgb0.t())/per_neural_region_num0, F.linear(neural_region_corr1.t(), pc_rgb1.t())/per_neural_region_num1
            #
            if current_growsp is not None:
                feats0, feats1 = F.linear(neural_region_corr0.t(), feats0.t()) / per_neural_region_num0, F.linear(neural_region_corr1.t(), feats1.t()) / per_neural_region_num1
                feats0, feats1 = F.normalize(feats0, dim=-1), F.normalize(feats1, dim=-1)

            for p in torch.unique(neural_region0):
                if p!=-1:
                    mask = p==neural_region0
            ...
            sp_size.append(per_neural_region_num0)
            ...
            all_sub_cluster[0].append(neural_region0)
            all_sub_cluster[1].append(neural_region1)
...
def get_pseudo(args, context, cluster_pred0, cluster_pred1, all_sub_cluster=None):
    ...
    for i in range(len(context)):
        ...
        sub_cluster_pred0 = all_sub_cluster[0][pc_no]+ region_num0
        sub_cluster_pred1 = all_sub_cluster[1][pc_no]+ region_num1
        valid_mask = (region != -1)
        ...
        pseudo0[valid_mask], pseudo1[valid_mask] = cluster_pred0[sub_cluster_pred0], cluster_pred1[sub_cluster_pred1]

        time_start = time.time()
        for p in np.unique(sub_cluster_pred0):
            if p != -1:
                mask = p == sub_cluster_pred0
        ...
        pc_no += 1
        tmp = np.unique(sub_cluster_pred0)
        region_num0 += len(tmp[tmp != -1])

        tmp = np.unique(sub_cluster_pred1)
        region_num1 += len(tmp[tmp != -1])
