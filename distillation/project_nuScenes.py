import torch, os, sys, math, pickle, argparse
import numpy as np
...
def get_args():
    ...
    parser.add_argument('--input_dir', type=str,  default='/home/zihui/HDD/LogoSP_release/data/nuScenes/',help='Where is the base logging directory')
    parser.add_argument('--output_dir', type=str,default='../data/nuScenes/dinov2s14/', help='Where is the base logging directory')
...
def extract_dinoV2_img_feature(img_dir, model):
    ...
    with torch.no_grad():
        ...
        features = features["x_norm_patchtokens"].cpu()
...
def process_one_scene(data_path, out_dir, args):
    ...
    torch.save({"feat": feat_bank[mask].half().cpu(), "mask_full": mask_entire}, join(out_dir, scene_id +'.pt'))
    print(join(out_dir, scene_id +'.pt') + ' is saved!')

    # pca = PCA(n_components=3)
    # pca_features = pca.fit_transform(feat_bank[mask].cpu().numpy())
    ...
    # #
    # write_ply(join(out_dir, scene_id +'.ply'),[locs_in[mask], feat_color],['x', 'y', 'z', 'red', 'green', 'blue'])
...
def main(args):
    ...
    data_root = join(args.input_dir, 'nuScenes_3d')
    data_root_2d = join(args.input_dir,'nuScenes_2d')
    ...
    for i in trange(total_num):
        ...
        scene_id = data_paths[i].split('/')[-1].split('.')[0]
