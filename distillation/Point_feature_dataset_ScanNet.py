...
class PointFeature(Dataset):
    def __init__(self, scene_id, scannet_2d_path):
        self.scene_id = scene_id
        self.SCANNET_FRAME_ROOT = scannet_2d_path + '/{}/{}/'
        self.frame_list = sorted([int(x.split(".")[0]) for x in os.listdir(self.SCANNET_FRAME_ROOT.format(self.scene_id, "color/"))])
        self.SCANNET_FRAME_PATH = scannet_2d_path +'/{}/'
        self.scale_factor = 4
        '''loading data frame'''
    ...
    def aug(self,color):
        ...
        # color = self.resize_crop_image(color, (1280, 960))
        ...
        color = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(torch.Tensor(color.astype(np.float32) / 255.0))
